...
module.exports = function generate(ctx) {
  ...
  **/
  ...
  function buildModelImports(model) {
    ...
    if (relations.length > 0) {
      relations.forEach((relationName, i) => {
        let targetClass = model.sharedClass.ctor.relations[relationName] ? model.sharedClass.ctor.relations[relationName].targetClass : model.sharedClass.ctor.settings.relations[relationName].model;
  ...
  function buildModelProperties(model, isInterface) {
    ...
    const alreadyBuiltRelations = Object.keys(model.sharedClass.ctor.relations);
    Object.keys(model.sharedClass.ctor.settings.relations).forEach(key => {
      if (alreadyBuiltRelations.includes(key)) {
        return;
      }
      const relation = model.sharedClass.ctor.settings.relations[key];
      const propertyName = key;// (relation.property??)
      const proxyObject = {};
      proxyObject[propertyName] = {targetClass: relation.model, type: relation.type};
      const relationType = buildRelationType({sharedClass: {ctor: {relations: proxyObject}}}, propertyName);
      let defaultTypeValue = !isInterface && ctx.defaultValue === 'enabled' && relationType.indexOf('Array') >= 0 ? ' = []' : '';
      defaultTypeValue = !isInterface && ctx.defaultValue === 'enabled' && relationType.indexOf('Array') === -1 ? ' = null' : defaultTypeValue;
      output.push( `  ${key}${isInterface ? '?' : ''}: ${relationType}${defaultTypeValue};` );
    });
    return output.join('\n');
  }
  /**
  ...
  function buildServiceImports(model, loadAccessToken, isIo) {
    ...
    getModelRelations(model).forEach((relationName, i) => {
      ...
      if (
        model.sharedClass.ctor.relations[relationName] && model.sharedClass.ctor.relations[relationName].modelThrough &&
  ...
  function buildNgModuleImports(models, environment, isIo, driver) {
    let imports = ['LoopBackAuth', 'SDKModels'];
  ...
  function buildMethodParams(model, methodName, params, isIo) {
    ...
    let availableClasses = relations.map((relationName, index) =>
      model.sharedClass.ctor.relations[relationName] ? model.sharedClass.ctor.relations[relationName].targetClass : model.sharedClass.ctor.settings.relations[relationName].model
    );
    ...
    relations.forEach(relationName => {
        if (model.sharedClass.ctor.relations[relationName] && model.sharedClass.ctor.relations[relationName].modelThrough) {
...
function getModelRelations(model) {
  let result = Object.keys(model.sharedClass.ctor.relations).filter(relationName =>
      model.sharedClass.ctor.relations[relationName].targetClass &&
